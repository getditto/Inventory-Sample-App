...
struct COrderByParam {
  const char *query_c_str;
  enum QuerySortDirection direction;
};

typedef const struct COrderByParam *COrderByParamArray;

typedef struct Ditto_Document **DocumentPtrPtr;
...
typedef struct Ditto_CollectionNames **CCollectionNamesPtrPtr;

typedef const struct AttachmentHandle **AttachmentHandlePtrPtr;
...
typedef const struct Ditto_ReadTransaction **ReadTxnPtrPtr;

typedef uintptr_t *USizePtr;

typedef const struct Ditto_WriteTransaction **WriteTxnPtrPtr;
...
int ditto_collection_exec_query_str(struct Ditto *ditto_raw,
                                    const char *coll_name,
                                    const struct Ditto_WriteTransaction *transaction,
                                    const char *query_c_str,
                                    COrderByParamArray order_by_params,
                                    unsigned int order_by_params_len,
                                    int limit,
                                    unsigned int offset,
                                    DocumentPtrPtr out_documents,
                                    unsigned int *out_doc_count);

int ditto_collection_get(struct Ditto *ditto_raw,
                         const char *coll_name,
                         const char *id,
                         const struct Ditto_ReadTransaction *transaction,
                         DocumentPtrPtr document);
...
int ditto_collections(struct Ditto *ditto_raw, CCollectionNamesPtrPtr collections);
...
int ditto_get_attachment_status(struct Ditto *ditto_raw,
                                const unsigned char *id_ptr,
                                uintptr_t id_len,
                                AttachmentHandlePtrPtr out_handle);

int ditto_get_collection_names(struct Ditto *ditto_raw, struct CStringVec *out_names);
...
void ditto_init_sdk_version(enum Platform platform, enum Language language, const char *sdk_semver);

/**
 * Prefer using ditto_live_query_str so you don't need to compile the queries first.
 */
...
int ditto_read_transaction(struct Ditto *ditto_raw, ReadTxnPtrPtr txn);
...
void ditto_vec_documents_free(DocumentPtrPtr docs, uintptr_t doc_count);

void ditto_vec_usizes_free(USizePtr usizes, uintptr_t usize_count);

int ditto_write_transaction(struct Ditto *ditto_raw, WriteTxnPtrPtr txn);
...
void free_c_string_vec(struct CStringVec *ptr);
...
struct CStringVec *new_c_string_vec(void);
